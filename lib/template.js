import { assert, clone, escapeHtml } from '@hapi/hoek';
import * as Formula from '@hapi/formula';
...
class _Template {
    ...
    static build(desc) {
        ...
        return new _Template(desc.template, desc.options || desc.functions ? { ...desc.options, functions: desc.functions } : undefined);
    }
...
_Template.prototype[Common.symbols.template] = true;
_Template.prototype.isImmutable = true;                // Prevents Hoek from deep cloning schema objects
...
internals.stringify = function (value, original, state, prefs, local, options = {}) {
    ...
    if (value instanceof Date) {
        return _Template.date(value, prefs);
    }
